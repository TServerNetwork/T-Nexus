...
/**
 * Detail GUI for per-item pickup and drop stats.
 */
public final class PlayerStatsItemDetailGui extends BaseGui {
     ...
     */
    public PlayerStatsItemDetailGui(
            TNexus plugin,
        ...
        super(
                ...
                plugin.getMessageConfig().getMessage(
                        ...
                        snapshot.targetName(),
                        plugin.getMessageConfig().getMessage("stats.labels.activity.item-breakdown")),
                ROWS);
    ...
    protected void buildContent() {
        ...
        setItem(
                ...
                createItem(
                        Material.CHEST,
                        getPlugin().getMessageConfig().getMessage("stats.labels.activity.item-breakdown"),
                        List.of(getPlugin().getMessageConfig().getMessage("stats.gui.category.icon.lore"))),
                null);
        setItem(FILTER_SLOT, createFilterItem(), event -> this.statsViewerManager.openItemDetailGui(
                this.viewer,
                this.target,
                this.periodFilter.next(),
                this.sortOrder));
        setItem(SORT_SLOT, createSortItem(), event -> this.statsViewerManager.openItemDetailGui(
                this.viewer,
                this.target,
                this.periodFilter,
                this.sortOrder.next()));

        List<StatsEntry> entries = this.snapshot.getSortedItemDetailEntries(this.sortOrder);
        if (entries.isEmpty()) {
            setItem(
                    EMPTY_STATE_SLOT,
                    createItem(
